...
func DelTargetLUNMap(targetName string) {
	globalSCSILUMap.mutex.Lock()
	defer globalSCSILUMap.mutex.Unlock()
	for _, device := range globalSCSILUMap.TargetsLUNMap[targetName] {
		if device.Storage != nil {
			_ = device.Storage.Close(device)
		}
		delete(globalSCSILUMap.AllDevices, device.UUID)
	}
	delete(globalSCSILUMap.TargetsLUNMap, targetName)
	delete(globalSCSILUMap.TargetsBSMap, targetName)
}
