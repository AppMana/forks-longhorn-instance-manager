...
// DeleteTarget removes a dynamically configured target without stopping the
// shared iSCSI listener. Existing sessions must be gone unless force is true.
func (s *ISCSITargetDriver) DeleteTarget(tgtName string, force bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.iSCSITargets[tgtName]; !ok {
		return fmt.Errorf("No such target: %s", tgtName)
	}
	if err := s.SCSI.DeleteTarget(tgtName, force); err != nil {
		return err
	}
	delete(s.iSCSITargets, tgtName)
	return nil
}
...
func (s *ISCSITargetDriver) Run(port int) error {
	...
	return s.RunListener(l)
}

// RunListener serves iSCSI on a listener created by the embedding process.
// Binding outside Run lets storage engines fail startup synchronously when a
// per-volume port is unavailable instead of racing readiness or exiting the
// entire process from library code.
func (s *ISCSITargetDriver) RunListener(l net.Listener) error {
