...
func (tgt *ISCSITarget) FindTPG(portal string) (uint16, error) {
	for tpgt, TPG := range tgt.TPGTs {
		for tgtPortal := range TPG.Portals {
			if portalsMatch(portal, tgtPortal) {
				return tpgt, nil
			}
		}
	}
...
func portalsMatch(local, configured string) bool {
	if strings.EqualFold(local, configured) {
		return true
	}
	localHost, localPort, localErr := net.SplitHostPort(local)
	...
	return ip != nil && ip.IsUnspecified() && net.ParseIP(localHost) != nil
}
